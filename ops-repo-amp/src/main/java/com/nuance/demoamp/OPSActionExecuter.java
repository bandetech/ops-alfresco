...
public class OPSActionExecuter extends ActionExecuterAbstractBase {
	...
	protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
	    ...
	    opsService.startJob(jobId, 1800, JobPriority.MEDIUM, params);
			...
			// Synchronized process, waiting for ops response...
			while(opsService.getJobInfo(jobId).getState() != JobStateEnum.COMPLETED){
				Thread.sleep(opsService.getJobInfo(jobId).getEstimatedWorkTimeSec() * 500);
			}
	    ...
	    // Create a file and get writer
			...
			if (point != -1){
				baseFileName = inputFileName.substring(0, point);
			}

			String outputFileName = baseFileName+".pdf";

			// Need to catch FileExistsException
	    FileInfo outFileInfo = fileFolderService.create(parent, outputFileName, ContentModel.TYPE_CONTENT);
